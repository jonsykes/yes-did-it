...
Every command supports `--agent`, which returns a markdown envelope
(`## Status / ## Result / ## Errors / ## Meta`) with semantic exit codes
(`NOT_FOUND=3`, `PERMISSION_DENIED=4`, `RATE_LIMITED=6`,
`VALIDATION_FAILED=8`). Mutations dry-run by default under `--agent`; pass
`--confirm` to execute. Other flags: `--fields` (column projection),
`--compact` (drops Errors/Warnings/Actions and inlines Meta),
`--cursor`/`--limit` (pagination, with `next_cursor` surfaced in Meta).

Run `ydi --agent describe` for the command index or `ydi <cmd> --agent
describe` for a single command's schema.

See [AGENT.md](AGENT.md) for the canonical reference, error-recovery
table, and common workflows.
