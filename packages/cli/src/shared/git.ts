...
export function branchLastCommitDate(name: string, cwd?: string): Date | null {
  const output = exec(`git log -1 --format=%cI ${name} --`, cwd)
...
export function getGitHooksDir(cwd?: string): string | null {
  return exec("git rev-parse --git-path hooks", cwd)
}

export function isHookInstalled(hookName: string, cwd?: string): boolean {
  const hooksDir = getGitHooksDir(cwd)
  if (!hooksDir) return false
  try {
    const { readFileSync } = require("fs")
    const { join, isAbsolute } = require("path")
    const hookPath = isAbsolute(hooksDir) ? join(hooksDir, hookName) : join(cwd || process.cwd(), hooksDir, hookName)
    const content = readFileSync(hookPath, "utf8")
    return content.includes("# ydi-hook")
  } catch {
    return false
  }
}
