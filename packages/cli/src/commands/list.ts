...
import { listTodos, listTodosPage, type Todo } from "../lib/api-client"
...
const DEFAULT_AGENT_FIELDS = ["id", "status", "text", "branch", "due", "tags"] as const
...
export default class List extends Command {
  ...
  static flags = {
    ...
    cursor: Flags.string({
      description: "Pagination cursor returned by previous --agent page",
    }),
    json: Flags.boolean({
  ...
  static agentMeta = {
    examples: [
      "ydi list --agent",
      "ydi list --agent --fields id,text,branch",
      "ydi list --agent --compact --limit 25",
      "ydi list --agent --cursor 50 --limit 50",
      "ydi list --agent --current-branch",
    ],
    related: ["ydi context --agent", "ydi standup --agent", "ydi sweep --agent"],
  }

  async run(): Promise<void> {
    const { flags } = await this.parse(List)
    const startedAt = Date.now()
    const command = buildCommandLine("ydi list", flags)
    ...
    if (flags["current-branch"]) {
      ...
      if (!detected) {
        return this.failVal(
          flags.agent,
          command,
          "Not in a git repository. Use --branch <name> to filter by a specific branch.",
          startedAt,
        )
      }
      branch = detected
    ...
    if (flags.here) {
      ...
      if (!detected) {
        return this.failVal(
          flags.agent,
          command,
          "Not in a git repository or no remote configured. Use --repo <url> to filter by a specific repo.",
          startedAt,
        )
      }
      repo = detected
      ...
      if (flags.stale) {
        ...
        if (remoteBranches.size === 0) {
          return this.failVal(
            flags.agent,
            command,
            "Could not reach remote. Cannot determine stale branches.",
            startedAt,
          )
        }
        todos = todos.filter(
      ...
      if (flags.agent) {
        this.log(
          renderListAgent({
            ...
            startedAt,
            scope: { branch, repo, active: flags.active, stale: flags.stale },
            compact: flags.compact,
            nextCursor,
            currentCursor: flags.cursor,
          }),
        )
        return
      }
...
function buildCommandLine(prefix: string, flags: Record<string, unknown>): string {
  const parts = [prefix]
  if (flags.agent) parts.push("--agent")
  if (flags.compact) parts.push("--compact")
  if (flags.fields) parts.push(`--fields ${flags.fields}`)
  if (flags.cursor) parts.push(`--cursor ${flags.cursor}`)
  return parts.join(" ")
}

interface RenderListAgentParams {
  command: string
  todos: Todo[]
  limit: number
  fieldsFlag?: string
  startedAt: number
  scope: { branch?: string; repo?: string; active: boolean; stale: boolean }
  compact?: boolean
  nextCursor: string | null
  currentCursor?: string
}

function renderListAgent(params: RenderListAgentParams): string {
  const { command, todos, limit, fieldsFlag, startedAt, scope, compact, nextCursor, currentCursor } = params
  const fields = parseFields(fieldsFlag) ?? [...DEFAULT_AGENT_FIELDS]
  const rows = todos.map((t) => projectTodo(t))
  const hasMore = nextCursor !== null
  const summary =
    todos.length === 0
      ? "0 results"
      : hasMore
        ? `${todos.length} results (more available — pass --cursor ${nextCursor})`
        : `${todos.length} results`

  const result =
    rows.length === 0 ? scopeEmptyMessage(scope) : formatMarkdownTable(rows, fields)

  const actions: string[] = []
  if (hasMore) {
    actions.push(
      `Next page: \`ydi list --agent --limit ${limit} --cursor ${nextCursor}\``,
    )
  }
  if (todos.length > 0) {
    const top = todos[0]
    const topShort = top.id.slice(0, 8)
    if (top.status === "pending") {
      actions.push(`Mark top item done (dry-run): \`ydi done ${topShort} --agent\``)
    }
  }
  actions.push("Show git context: `ydi context --agent`")

  const meta: Record<string, string> = {
    fields: `fields: ${fields.join(",")}`,
    total: `${todos.length} total`,
  }
  if (currentCursor) meta.cursor = `cursor: ${currentCursor}`
...
function scopeEmptyMessage(scope: {
  ...
  if (scope.active) return "No active todos found."
  if (scope.stale) return "No stale todos found."
  if (scope.branch) return `No todos found on branch ${scope.branch}.`
  if (scope.repo) return `No todos found in repo ${scope.repo}.`
