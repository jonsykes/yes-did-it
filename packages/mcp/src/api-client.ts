...
/** Find a todo by full UUID or short prefix. Throws with the same messages
 *  as resolveId so error classification stays consistent. Used for dry-run
 *  previews where we need the full record, not just the id. */
export async function findTodoByIdOrPrefix(id: string): Promise<Todo> {
  const todos = await listTodos({ status: "all", limit: 200 })
  const matches =
    id.length === 36
      ? todos.filter((t) => t.id === id)
      : todos.filter((t) => t.id.startsWith(id))
  if (matches.length === 0) throw new Error(`No todo found matching "${id}"`)
  if (matches.length > 1)
    throw new Error(
      `Ambiguous ID "${id}" — ${matches.length} todos match. Use more characters.`
...
export async function listTodos(params: ListTodosParams): Promise<Todo[]> {
  const page = await listTodosPage(params)
  return page.todos
}

export async function listTodosPage(
  params: ListTodosParams,
): Promise<{ todos: Todo[]; cursor: string | null }> {
  ...
  const data = await json<{ todos: Todo[]; cursor: string | null }>(res)
  return { todos: data.todos, cursor: data.cursor ?? null }
}
