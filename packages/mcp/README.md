...
Every tool returns a markdown envelope (`## Status / ## Result / ## Errors
/ ## Meta`). Mutating tools (`add_todo`, `complete_todo`, `delete_todo`,
`update_todo`) require `confirm: true` — without it, the tool returns a
dry-run envelope describing the target without touching state. `list_todos`
supports cursor pagination via the `cursor` param (the previous response's
Meta surfaces `next_cursor`).

See [AGENT.md](AGENT.md) for the canonical reference: error-recovery
table, structured error fields (`Code`, `Message`, `Retryable`,
`Suggestion`), and workflow recipes.
